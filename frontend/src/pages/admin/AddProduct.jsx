      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-10 border-b border-white/10 pb-6">
          <span className="text-brand-accent text-[10px] font-bold tracking-[0.5em] uppercase mb-4 block flex items-center gap-2">
            <Sparkles size={14} /> Catalog Expansion
          </span>
          <h1 className="text-3xl md:text-4xl font-serif text-white italic">Add New Fragrance</h1>
        </div>
        ...
        <form onSubmit={submitHandler} className="bg-[#0a0a0a] border border-white/5 p-8 shadow-2xl space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" name="name" required value={formData.name} onChange={handleChange} className="w-full bg-transparent border-b border-white/20 text-white pb-2 focus:outline-none focus:border-brand-accent transition-colors" placeholder="ex: Bleu de Chanel" />
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-gray-500 font-bold mb-2">Brand</label>
              <input type="text" name="brand" required value={formData.brand} onChange={handleChange} className="w-full bg-transparent border-b border-white/20 text-white pb-2 focus:outline-none focus:border-brand-accent transition-colors" placeholder="ex: Chanel" />
            </div>
          </div>
          ...
          {/* IMAGE UPLOAD FINAL (Bouton Uniquement) */}
          <div>
            <label className="block text-[10px] uppercase tracking-widest text-gray-500 font-bold mb-2">Fragrance Image (Upload Only)</label>
          ...
          <div>
            ...
            <textarea name="description" required value={formData.description} onChange={handleChange} rows="4" className="w-full bg-transparent border border-white/20 text-white p-4 focus:outline-none focus:border-brand-accent transition-colors text-sm" placeholder="Describe the fragrance notes..."></textarea>
          </div>
          ...
          <div className="pt-6">
            <button type="submit" disabled={loading} className="w-full bg-brand-accent text-black py-4 text-xs font-bold uppercase tracking-[0.2em] hover:bg-white transition-colors flex justify-center items-center gap-2">
              {loading ? 'Creating...' : <><Save size={16} /> Publish Fragrance</>}
            </button>
          </div>
